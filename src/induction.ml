...
let test_normalize () =
  ...
  let rec print_term t =
    ...
    | Constr (j, d, args) -> Printf.printf "%s%d [%d args]" d.name j (List.length args)
    | App (f, arg) -> Printf.printf "App ("; print_term f; Printf.printf ", "; print_term arg; Printf.printf ")"
    | Elim (d, p, cases, t') -> Printf.printf "Elim %s (...)" d.name
    | _ -> Printf.printf "Other"
  in
  Printf.printf "Normalized: "; print_term normal; print_endline "";
  let even_term = App (to_even, two) in
  let even_normal = normalize env_mutual empty_ctx even_term in
  Printf.printf "ToEven: "; print_term even_normal; print_endline ""
